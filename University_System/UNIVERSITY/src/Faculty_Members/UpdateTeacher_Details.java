...
public class UpdateTeacher_Details extends JDialog implements ActionListener{
    //Panel
    ...
    public UpdateTeacher_Details() {
        setTitle("Update Details(Faculty)");
        setSize(950, 700);
        setLocation(320, 100);
        setDefaultCloseOperation(DISPOSE_ON_CLOSE);
        setResizable(false);
        getContentPane().setBackground(new Color(183, 0, 0));
        setModal(true);
        setLayout(null);
    ...
    private void addComponents() {
        ...
        // heading
        heading_label = new JLabel("Update Teacher Details");
        heading_label.setBounds(320, 0, 500, 50);
        heading_label.setForeground(new Color(183, 0, 0));
        ...
        JLabel lblroll = new JLabel("Select Employee ID:");
        ...
        // roll
        roll_label = new JLabel("Employee ID:");
       ...
       tfCourse = new JTextField();
        ...
        try {
          Conn conn = new Conn();  
          String query = "select * from faculty where empID = '"+crollno.getSelectedItem()+"'";
          ResultSet rs = conn.s.executeQuery(query);
          while(rs.next()){
            namlabel.setText(rs.getString("name"));
            father_namelabel2.setText(rs.getString("fname"));
            roll2.setText(rs.getString("empID"));
    ...
    if (e.getSource() == update) {
        ...
        try {
            if (address.equals("") && ph.equals("") && email.equals("")  && qualification.equals("") && department.equals("")) {
                JOptionPane.showMessageDialog(UpdateTeacher_Details.this,"Empty fields!");
            }  else{
                String query = "UPDATE faculty SET address='"+address+"', ph='"+ph+"', email='"+email+"', qualification='"+qualification+"', department='"+department+"' WHERE empID = '"+roll+"'";
                Conn con = new Conn();
                con.s.executeUpdate(query);
                JOptionPane.showMessageDialog(UpdateTeacher_Details.this, "<html><body style=\\'color:green;\\'><b>Teacher Details Updated Successfully</b></body></html>");
                UpdateTeacher_Details.this.dispose();
            }
