...
public class StudentLeaveDetails extends JDialog implements ActionListener{
    private Choice crollno;
    private JTable Table;
    private JButton search, print, cancel;

    public StudentLeaveDetails() {
        setTitle("Leave Details (Student)");
        setSize(900, 700);
        setLocation(300, 100);
    ...
    private void addComponents() {
        ...
        try {
            Conn conn = new Conn(); 
            ResultSet rs = conn.s.executeQuery("Select * From student_leave");
            while (rs.next()) {
